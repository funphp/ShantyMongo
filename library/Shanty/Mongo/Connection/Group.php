...
class Group
{
	...
	{
		$this->_masters = new Stack();
		$this->_slaves = new Stack();
	...
	{
		if ($connectionOptions instanceof \Zend\Config) {
		...
		$group = $this;
		$addConnections = function(Stack $stack, array $connections) use ($group) {
			foreach ($connections as $connectionData) {
				$options = array_intersect_key($connectionData, array_flip(Connection::getAvailableOptions()));

				$connection = new Connection($group->formatConnectionString($connectionData), $options);
	 ...
	 * @param int $weight
	 */
	public function addMaster(Connection $connection, $weight = 1)
	{
	 ...
	 *
	 * @return Shanty\Mongo\Connection\Stack
	 */
	 ...
	 * @param $weight
	 */
	public function addSlave(Connection $connection, $weight = 1)
	{
