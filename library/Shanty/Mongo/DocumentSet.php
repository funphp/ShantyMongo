...
use Shanty\Mongo\Document as Document,
 ...
 * @author     Coen Hyde
 */
class DocumentSet extends Document
{
	const DYNAMIC_INDEX = '$';
	...
	public function getProperty($index = null)
	{
		...
		if ($new && $this->hasRequirement(static::DYNAMIC_INDEX, 'AsReference')) {
			throw new ShantyException("Can not create a new document from documentset where document must be saved as a reference");
		}
	...
	public function setProperty($index, $document)
	{
		...
		if (!$new && !is_numeric($index)) {
			throw new ShantyException("Index must be numeric '{$index}' given");
		}
...
//		$value = $this->getFilters(self::DYNAMIC_INDEX)->filter($document);
	 ...
	 * @param Shanty\Mongo\Document $document
	 */
	public function pushDocument(Document $document)
	{
	...
	public function __call($name, $arguments = array())
	{
		...
		throw new ShantyException("Captured in __call. Method $name does not exist.");
	}
}
